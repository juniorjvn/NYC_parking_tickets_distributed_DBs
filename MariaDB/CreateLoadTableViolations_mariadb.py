...
# Bronx violations records - 2021
# will be store in a MariaDB database
borough = 'BX'
borough_id = 2
year = '2021'
# Set the number of row to be inserted into the table
# total records in 2021 - > 1946040
limit = 1946040
...
# Connect to the nyc_open_data_bronx database and create a table
conn = create_mariadb_connection(user=user, password=password, database=database)
cur = conn.cursor()

create_violations_table_stm = '''
CREATE TABLE IF NOT EXISTS violations(
    summons_number SERIAL,
    state VARCHAR(2),
    county INTEGER,
    plate VARCHAR(10),
...
insert_stm = '''INSERT IGNORE INTO violations({}) VALUES({});'''
...
for violation in parking_cam_violations:
    ...
    if valid_date:
        ...
        try:
            ...
            conn.rollback()
            # conn.close()
            print('Error inserting values {} into MariaDB database.'.format(violation))
            print('Error:', e)
            # exit()
